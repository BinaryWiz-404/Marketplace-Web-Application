...
@login_required   #it is like a decorator like app.route which ensure that after login only user come to this page
def market_page():
...
def register_page():
    ...
    if form.validate_on_submit():
        user_to_create=User(username=form.username.data,
                            ...
                            password_fun=form.password1.data)
        db.session.add(user_to_create)
        db.session.commit()
        login_user(user_to_create)  #this will directly direct new user to market page instead of login page
    ...
    if form.errors != {}: #if there are not errors from the validations
        for err_msg in form.errors.values():
           flash(f'There was an error with creating a user:{ err_msg }',category='danger')#in place of print use flash to show the error messages in the web page
...
def logout_page():
    logout_user() #this is enough to grab the login 
